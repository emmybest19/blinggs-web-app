...
import { notFoundSuggestions } from '../router/navigation'
...
export default function NotFoundPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f1b2d] via-[#1a2744] to-[#0b1220] flex items-center justify-center p-4 overflow-hidden">
      ...
      <div className="absolute top-10 left-10 w-72 h-72 bg-[#00D4AA] rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-morph" />
      <div
        className="absolute bottom-10 right-10 w-72 h-72 bg-indigo-600 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-morph"
        style={{ animationDelay: '2s' }}
      />
      <div
        className="absolute top-1/2 left-1/2 w-72 h-72 bg-emerald-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-morph"
        ...
        <div className="mt-16 pt-8 border-t border-white/10 animate-fade-in-up-delay-4">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {notFoundSuggestions.map((link) => (
              ...
              >
                {link.label}
                <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-[#00D4AA] group-hover:w-full transition-all duration-300" />
              </button>
            ))}
          </div>
        </div>
